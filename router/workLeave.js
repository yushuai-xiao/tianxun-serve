...
// 获取假期数据
router.get("/attendance/leave/list", async (req, res, next) => {
  let {
		user_id,
		user_name,
		department,
  ...
  try {
    ...
    console.log("获取请假列表时发生错误:", e.message);
    next("系统错误");
  }
})

// 新增假期数据
router.post("/attendance/leave", async (req, res, next) => {
  let {
		reason,
		start,
		end,
	...
	let checkSql = `select id from schedule where 
									 reason = '${reason}' and 
									 start = '${start}' and 
									 end = '${end}'  and type = '0' and status = 0`
	try {
  ...
  let sql = batch(() => {
    let sql = `
      insert into schedule(reason,start,end,status,type,user_id) 
			      values ('${reason}','${start}','${end}','${0}','${0}','${user_id}') 
      `
    return sql;
  })
  try {
    ...
    console.log("请假申请时发生错误:", e.message);
    next("系统错误");
  }
})

// 批准和不批准请假
router.patch("/attendance/leave/:id", async (req, res, next) => {
  let { id } = req.params;
  if (!id) {
    next();
    return;
...
// 删除假期
router.delete("/attendance/leave/:id", async (req, res, next) => {
